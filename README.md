...
A minimalist, zero-dependency PostgreSQL queuing library for Node.js. Built for high performance and simplicity, designed to be used with a `postgres.js` instance.
...
- [Advanced Features (Planned)](#advanced-features-planned)
  ...
  - [Listener Heartbeat](#listener-heartbeat--health-monitoring-planned)
  - [Delayed Jobs](#delayed-jobs-planned)
  - [Job Retries](#job-retries-with-exponential-backoff-planned)
...
### Core
- **Zero Dependencies**: Lightweight and fast. Optional runtime validation using your choice of Standard Schema-compliant library.
...
This release changes the queue storage model:
- `queue.id` is now `uuid` (generated by PostgreSQL).
- `queue.channel` is persisted and used for channel-specific fetching.
- `broadcasts` table is now used for broadcast fan-out with `LISTEN/NOTIFY`.
...
### Migration Guide (v0.x -> v1.x API)

```typescript
// Before
const queue = Queue(sql, { channel: 'emails' });
await queue.push<EmailJob>({ to: 'user@example.com', body: 'Hello' });
await queue.listen<EmailJob>(async (payload) => {
  // ...
});

// After
const queue = Queue<EmailJob>(sql, { channel: 'emails' });
await queue.enqueue({ to: 'user@example.com', body: 'Hello' });
await queue.listen(async (payload, job) => {
  // payload is EmailJob, job is strongly typed metadata
});
```

What changed:
- Bind payload type at queue creation (`Queue<EmailJob>(...)` or `Queue(sql, { schema })`).
- Use `enqueue()` (the old `push()` remains as a compatibility alias).
- Remove per-call generics from `listen()` and `enqueue()`.
- Broadcasts now persist to `broadcasts` and notify listeners by channel.
...
- `options` (optional):
  ...
  - `runAt` (Date, planned): Schedule the job to run at a specific time
  - `delay` (number, planned): Delay execution by this many milliseconds
...
**Behavior:**
- Uses PostgreSQL `LISTEN/NOTIFY` for instant wakeup (no polling)
...
## Listener Heartbeat & Health Monitoring (Planned)
...
const activeListeners = await sql`
  SELECT listener_id, channel, last_seen_at 
...
## Job Retries with Exponential Backoff (Planned)
...
interface EnqueueOptions {
  ...
  // Schedule for future execution (planned)
  runAt?: Date;
  delay?: number; // ms
}
